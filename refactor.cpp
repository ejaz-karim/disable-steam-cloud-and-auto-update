...
string getConfigDirectory()
{
    ...
    return sharedconfig_file;
}

ifstream getConfigFile(string sharedconfig_file)
{
    ifstream sharedconfig_if(sharedconfig_file);                                      // Input file stream created for the main config
    return sharedconfig_if;
}


ifstream getLibraryFile()
{
...
stringstream getAppIds(ifstream& library_if)
{
...
int editConfigFile(ifstream& sharedconfig_if, stringstream& appIds, string& configDirectory)
{
    string line;
    ...
    bool appsBlockReached = false;
        ...
        while (getline(sharedconfig_if, line))
        {
            ...
            if (line.find("apps") != string::npos)
            {
                ...
                while (getline(appIds, sharedconfig_idLine))
                {
        ...
        ofstream sharedconfig_of(configDirectory);
...
{    
    ...
    ifstream libraryFile = getLibraryFile();
    ifstream configFile = getConfigFile(configDirectory);

    stringstream libraryStringToCheck;
    libraryStringToCheck = libraryFile.rdbuf();

    if(libraryStringToCheck.str().find("apps") == string::npos)
    {
    ...
    stringstream appIds = getAppIds(libraryFile);

    stringstream configStringToCheck;
    configStringToCheck = configFile.rdbuf();

    if(configStringToCheck.str().find("apps") == string::npos)
    {
    ...
    editConfigFile(configFile, appIds, configDirectory);
}